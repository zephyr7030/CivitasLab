...
def make_cfg(max_turns=1000, init_pop=5):
    ...
    # dev37 is diagnostic-only: keep experimental dividends off by default.
...
if __name__=='__main__':
    payload=[]
    payload.append({'label':'dev37_5p_10_detail_seed20260517','result':run_one((20260517,10,5,True))})
    payload.append(suite('dev37_baseline_5p_1000',1000,5))
    payload.append(suite('dev37_baseline_10p_1000',1000,10))
    # Collect full low-pop tail details for known/representative seeds.
    for seed, init_pop in [(20260517,5),(1,5),(42,5),(88,10)]:
        payload.append({'label':f'dev37_tail_detail_seed{seed}_{init_pop}p_1000','result':run_one((seed,1000,init_pop,True))})
    with open('BOT8_dev37_TEST_RESULTS.json','w',encoding='utf-8') as f:
