...
def run_one(args):
    ...
    return {
        ...
        'food_release_pressure':sum_col('FoodCompanyHardNeedPressure'),
        'food_release_listed':sum_col('FoodCompanyHardNeedReleaseListed'),
        'food_sellable_stock':sum_col('FoodCompanySellableStock'),
        'company_hard_release_count':sum_col('CompanyHardNeedReleaseEnabledCount'),
...
def suite(label,max_turns,init_pop,seeds=SEEDS):
    ...
    return {'label':label,'settings':{'max_turns':max_turns,'init_pop':init_pop},'summary':{
        ...
        'avg_food_release_pressure':mean('food_release_pressure'),
        'avg_food_release_listed':mean('food_release_listed'),
        'avg_food_sellable_stock':mean('food_sellable_stock'),
        'avg_company_hard_release_count':mean('company_hard_release_count'),
...
if __name__=='__main__':
    ...
    for max_turns,init_pop in [(10,5),(100,5),(1000,5),(100,10),(1000,10)]:
        result=suite(f'dev41_{init_pop}p_{max_turns}',max_turns,init_pop)
        payload.append(result)
        print(result['label'], result['summary'])
    with open('BOT8_dev41_TEST_RESULTS.json','w',encoding='utf-8') as f:
